...
def main():
    ...
    parser.add_argument("-i", "--input", dest="hichip-peaks_results",action="store",required=True,
    ...
    hichip-peaks_results = os.path.abspath(args.hichip-peaks_results)
    ...
    if not os.path.isdir(hichip-peaks_results):
    ...
    input_files = [fn for fn in os.listdir(hichip-peaks_results) if fn.endswith("diffpeak_data.pickle")]
    ...
    for current_file in input_files:
        with open(os.path.join(hichip-peaks_results,current_file), "rb") as current_pickle:
